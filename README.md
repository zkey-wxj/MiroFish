...
1. 进入历史记录即触发回放，前端不做“可回放”判断，直接按后端返回数据渲染。
2. Step3 回放数据来源于运行动作日志（对应接口 `/api/simulation/<simulation_id>/run-status/detail`）。
3. Step3 若无历史数据，会自动回退到当前“启动模拟”逻辑。
4. 回放模式且有历史数据时，会显示“重新模拟”按钮，点击后将归档历史日志并强制重启。
5. Step5 回放依赖报告数据、Report Agent 对话历史与采访历史，存在即展示，不存在展示空态。
...
git fetch upstream pull/{pr-id}/head
git checkout main
